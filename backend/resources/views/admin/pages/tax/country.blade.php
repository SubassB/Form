...
@section('content')
        ...
        <div class="col-md-12">
            <div class="customers__area bg-style mb-30">
                <div class="item-title">
                    <div class="col-xs-6">
                        <button data-bs-toggle="modal" data-bs-target="#createModal1" class="btn btn-md btn-info">{{ __('Add')}}</button>
                    </div>
                </div>
                ...
                <form enctype="multipart/form-data" method="POST" action="{{route('admin.country_tax_store')}}">
                    <div class="modal-body">
                        ...
                        <div class="input__group mb-25">
                            ...
                            <input type="number" min="0" step="0.01" name="percentage" placeholder="{{__('Tax Rate')}}" required>
                        </div>
                    </div>
    ...
    @foreach ($taxes as $tax)
        <div class="modal fade" id="editModal{{$tax->id}}" tabindex="-1" role="dialog" aria-labelledby="editModalTitle{{$tax->id}}" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header bg-success">
                    ...
                    <form enctype="multipart/form-data" method="POST" action="{{route('admin.country_tax_update', encrypt($tax->id))}}">
                        <div class="modal-body">
                            ...
                            <div class="input__group mb-25">
                                <label for="exampleInputEmail1">{{ __('Tax Rate (In Percentage)')}}</label>
                                <input type="number" min="0" step="0.01" name="percentage" placeholder="{{__('Tax Rate')}}" value="{{$tax->percentage}}" required>
                            </div>
                            <div class="input__group mb-25">
                                <label for="status">{{__('Status')}}</label>
                                <select name="status" id="status">
                                    <option value="{{ACTIVE}}" {{$tax->status == ACTIVE ? 'selected' : ''}}>{{__('Active')}}</option>
                                    <option value="{{INACTIVE}}" {{$tax->status == INACTIVE ? 'selected' : ''}}>{{__('Inactive')}}</option>
                                </select>
                            </div>
                        </div>
                        <div class="modal-footer">
