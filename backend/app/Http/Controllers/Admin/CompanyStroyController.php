...
class CompanyStroyController extends Controller
{
    public function companyStory(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->rawColumns(['action', 'en_Description'])
                ->make(true);
        }
    ...
    public function companyStoryStore(CompanyStoryRequest $request)
    {
        ...
        if ($story) {
            return redirect()->route('admin.company.story')->with('toast_success', __('Successfully Stored !'));
        }
    ...
    public function companyStoryDelete($id)
    {
        ...
        if ($delete) {
            $delete->delete();
            return redirect()->route('admin.company.story')->with('toast_success', __('Successfully Deleted !'));
        }
        return redirect()->route('admin.company.story')->with('toast_error', __('Does Not Delete!'));
    }
    ...
    public function companyStoryUpdate(Request $request)
    {
        ...
        if ($story) {
            return redirect()->route('admin.company.story')->with('toast_success', __('Successfully Updated !'));
        }
        return redirect()->route('admin.company.story')->with('toast_danger', __('Something is wrong !'));
    }
}
