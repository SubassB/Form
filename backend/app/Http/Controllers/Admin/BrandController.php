        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($data) {
                    ...
                    $btn = $btn.'<a href="' . route('admin.brand.edit', $data->id) . '" class="btn-action" title="Edit"><i class="fas fa-pencil-alt"></i></a>';

                    if($data->Status == 1){
                        $btn = $btn.'<a href="' . route('admin.brand.inactive', $data->id) . '" class="btn-action" title="Inactive"><i class="fas fa-arrow-down"></i></a>';
                    }else{
                        $btn = $btn.'<a href="' . route('admin.brand.active', $data->id) . '" class="btn-action" title="Active"><i class="fas fa-arrow-up"></i></a>';
                    }
...
//        return $request->all();
            ...
            if ($brand) {
                return redirect()->route('admin.brand')->with('toast_success', __('Successfully Stored !'));
            }
    ...
    public function brandDelete($id){
            ...
            if ($delete) {
                $delete->delete();
                return redirect()->route('admin.brand')->with('toast_success', __('Successfully Deleted !'));
            }
            return redirect()->route('admin.brand')->with('toast_error', __('Does Not Delete!'));
    }

    public function brandActive($id){
        $inactive=Brand::find($id)->update(['Status'=>1]);
        if($inactive){
            return redirect()->route('admin.brand')->with('toast_success', __('Successfully Active !'));
        }
        return redirect()->route('admin.brand')->with('toast_success', __('Does not Updated !'));
    }
    public function brandInactive($id){
        $inactive=Brand::find($id)->update(['Status'=>0]);
        if($inactive){
            return redirect()->route('admin.brand')->with('toast_success', __('Successfully Inactive !'));
        }
        return redirect()->route('admin.brand')->with('toast_success', __('Does not Updated !'));
    }
    ...
    public function brandUpdate(Request $request){
        ...
        if ($update) {
            return redirect()->route('admin.brand')->with('toast_success', __('Successfully Update !'));
        }
