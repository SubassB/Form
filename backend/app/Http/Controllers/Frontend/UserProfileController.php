...
class UserProfileController extends Controller
{
    public function userProfile()
    {
        ...
        $data['billing'] = Billing::where('User_Id', $authId)->first();
    ...
    public function userProfileUpdate(Request $request)
    {
        ...
        if ($user) {
            return redirect()->back()->with('toast_success', __('Successfully Stored !'));
        }
        return redirect()->back()->with('toast_success', __('Something is problem !'));
    }
    public function myOrder()
    {
        $authId = Auth::user()->id;
        $orders = Order::where('user_id', $authId)->with('order_details', 'order_details.product')->latest()->get();
    ...
    public function reviewStore(ReviewStoreRequest $request)
    {
        ...
        if (!empty($prev_review)) {
            ...
            if (!empty($update)) {
                return redirect()->back()->with('toast_success', 'Your review is successfully updated!');
            }
        }
        ...
        if (!empty($store)) {
            return redirect()->back()->with('toast_success', 'Review successful');
        }
        return redirect()->back()->with('toast_error', 'Something went wrong!');
    }

    public function userChangeBilling(Request $request)
    {
        $authId = Auth::user()->id;
        $user = Billing::where('User_id', $authId)->update([
            'Name' => $request->billing_name,
            'Email' => $request->billing_email,
            'phone_no' => $request->billing_phone,
        ...
        if ($user) {
            return redirect()->back()->with('toast_success', __('Successfully Saved !'));
        }
        return redirect()->back()->with('toast_success', __('Something is problem !'));
    }
    ...
    {
        $data['wishes'] = Wishlist::where('User_Id', Auth::id())->with('products')->get();
        $data['title'] = __('Wish List');
        $data['description'] = __('Wish List');
        $data['keywords'] = __('Wish List');
        return view('front.pages.user_profile.my_wishlist', $data);
    }
}
