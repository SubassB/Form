...
if (!function_exists('Category')) {
    function Category()
    {
        return Category::where('status', 1)->where('parent_category', 0)->get();
    }
}
if (!function_exists('CategoryMenu')) {
    function CategoryMenu()
    {
        return Category::where('status', 1)->where('Top_Menu', 1)->orderBy('Menu_Pos', 'ASC')->get();
    }
}
...
if (!function_exists('UploadImage')) {
    ...
    {
        return 'uploaded_files/upload_images/';
    }
}
...
if (!function_exists('checkBoxValueApi')) {
    function checkBoxValueApi($value = null)
    {
...
if (!function_exists('getwishListCount')) {
    function getwishListCount()
    {
        if (Auth::check()) {
            ...
            return session()->has('wishlist') ? count(session()->get('wishlist')) : '0';
        }
    }
}
...
if (!function_exists('subCategoryMenu')) {
    function subCategoryMenu($category_id)
    {
        $categories = Category::where('Parent_Category', $category_id)->get();
        $html = '';
        if (count($categories) >= 0) {
            $html .= '<ul class="menu-items">';
            foreach ($categories as $item) {
                $html .=  "<li class='mega-menu-items'><a href='" . route('category.product', $item->id) . "' class='mega-menu-link'>$item->en_Category_Name</a></li>";
            }
            $html .= '</ul';
        }
...
if (!function_exists('subCategoryMobileMenu')) {
    function subCategoryMobileMenu($category_id)
    {
        $categories = Category::where('Parent_Category', $category_id)->get();
        $html = '';
        if (count($categories) >= 0) {
            ...
            foreach ($categories as $item) {
                $html .=  "<li class='sub-menu-item'><a href='" . route('category.product', $item->id) . "' class='sub-menu-link'>$item->en_Category_Name</a></li>";
            }
            $html .= '</ul';
        }
